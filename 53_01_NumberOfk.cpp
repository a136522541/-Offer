...
int FindLeftEdge(int Array[], int First, int Last, int Num){
  ...
  while(First <= Last){
    Mid = (First + Last)/2;
    if(Array[Mid] >= Num) Last = Mid - 1;
    else First = Mid + 1;
  }

  return Array[First]==Num ? First : First + 1;
...
int NumberOfK(int Array[],int len, int Num){
  ...
  while(P1 <= P2){
    ...
    else if(Array[PMid] > Num) P2 = PMid - 1;
    else if(Array[PMid] < Num) P1 = PMid + 1;
  }
